# Разведка: Dynamite (клон Dynamite)

Репозиторий: `/Users/user/Dev/vibecode/Pocket/Dynamite`  
...
| Факт | Координаты |
|------|------------|
| Xcode project | `Dynamite.xcodeproj` |
| Workspace | `Dynamite.xcodeproj/project.xcworkspace` (SPM) |
| Targets | `Dynamite` (app), `DynamiteXPCHelper` (xpc) — `Dynamite.xcodeproj/project.pbxproj:762–821` |
| App product | `Dynamite.app`; `productName = dynamicNotch` — pbxproj:818–819 |
| Bundle ID app | `theboringteam.dynamite` — pbxproj:1252 / 1305 |
| Bundle ID XPC | `theboringteam.dynamite.DynamiteXPCHelper` — pbxproj:1053 / 1078 |
| Shared `.xcscheme` в репо | **нет** (нет `xcshareddata/xcschemes/`) — схема по умолчанию совпадает с target name `Dynamite` (автоген Xcode) |
...
### SPM-продукты, прилинкованные к target `Dynamite` (pbxproj:804–817)
...
### Entry
- `@main struct DynamicNotchApp: App` — `Dynamite/DynamiteApp.swift:15–16`
- `AppDelegate` через `@NSApplicationDelegateAdaptor` — `DynamiteApp.swift:17`, класс:52
- MenuBarExtra «Dynamite» — `DynamiteApp.swift:32–48`
- Lifecycle: `applicationDidFinishLaunching` — `DynamiteApp.swift:282+`

### Создание notch-окна
- `createDynamiteWindow(for:with:)` — `DynamiteApp.swift:234–265`
  - `DynamiteSkyLightWindow` (subclass `NSPanel`) — `Dynamite/components/Notch/DynamiteSkyLightWindow.swift:34`
...
- Размеры: `openNotchSize` / `windowSize` — `Dynamite/sizing/matters.swift:16–17` (640×190 / +shadow)
- UI root: `ContentView` — `Dynamite/ContentView.swift`
...
Каталог `Dynamite/components/Shelf/`:
...
| Services | `ShelfDropService.swift`, `ShelfPersistenceService.swift` (Application Support `Dynamite/Shelf/items.json`), `ShelfActionService.swift` (copy to NSPasteboard), `QuickLookService`, `QuickShareService`, `ShareServiceFinder`, `TemporaryFileStorageService`, `ThumbnailService`, `ImageProcessingService` |
...
- Команда: `xcodebuild -list -project Dynamite.xcodeproj`
...
Targets из pbxproj (если resolve пройдёт): `Dynamite`, `DynamiteXPCHelper`.
...
```
DynamiteApp.swift          → @main + AppDelegate + window create + hotkeys
