# Iteration 3 report — Dynamite clipboard
...
| File | What |
|------|------|
| `Dynamite/components/ClipboardHistory/Views/ClipboardKeyboardMonitor.swift` | Physical keyCode nav (Carbon kVK_ANSI_*), ⌘C, Space/Enter/Delete/Escape by keyCode |
| `Dynamite/components/ClipboardHistory/Views/ClipboardHistoryView.swift` | External popup wire-up, onCopy → `copySelected()`, close on notch/tab dismiss |
| `Dynamite/components/ClipboardHistory/Views/ClipboardTextPopupPanel.swift` | **New** — non-activating floating panel + controller |
| `Dynamite/components/ClipboardHistory/Core/ClipboardHistoryManager.swift` | `migrateHistorySizeCapIfNeeded()` |
| `Dynamite/models/Constants.swift` | `clipboardHistorySize` default **9**; migration flag `clipboardHistorySizeCappedTo9` |
| `Dynamite/components/Settings/SettingsView.swift` | History size Stepper **1…9** |
| `Dynamite/ContentView.swift` | Hide text popup when notch closes / leave clipboard tab |
| `Dynamite.xcodeproj/project.pbxproj` | Register `ClipboardTextPopupPanel.swift` |
...
- **Position:**
  1. Find visible `DynamiteSkyLightWindow` / `DynamiteWindow` (else main screen).
...
```
pkill -9 -x Dynamite
rm -rf /Applications/Dynamite.app
ditto /tmp/pocket-dd/Build/Products/Debug/Dynamite.app /Applications/Dynamite.app
open /Applications/Dynamite.app
```
...
command=/Applications/Dynamite.app/Contents/MacOS/Dynamite
TXT=/Applications/Dynamite.app/Contents/MacOS/Dynamite
PATH_OK ALIVE_OK  (≥5s)
```
