...
//  Dynamite — continuous rate-limit polling ported from Orca rate-limits/service.ts
//
...
final class RateLimitService: ObservableObject {
    ...
    private let cacheURL: URL = {
        ...
        let dir = base.appendingPathComponent("Dynamite/UsageAuth", isDirectory: true)
    ...
    private func refreshIfNeeded(force: Bool) async {
        ...
        // Claude keeps its OAuth session in Keychain. Export the newest blob
        // before reading it so a regular usage refresh does not depend on the
        // user pressing "Sync Claude session" manually. The bridge is a
        // background child process and never launches Terminal.
        _ = await ProviderAuthService.refreshClaudeCredentialsForUsage()
