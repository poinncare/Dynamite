...
//  Dynamite — provider sign-in without Gatekeeper .command dialogs.
//
//  Flow:
...
//  (that triggers the "Dynamite wants Claude Code-credentials" dialog).
//  The hidden Terminal bridge exports the keychain blob to:
//    ~/.claude/.credentials.json
//    ~/Library/Application Support/Dynamite/UsageAuth/claude.credentials.json
//
...
enum ProviderAuthService {
    ...
    /// Refresh the exported Claude session for an usage fetch.
    ///
    /// This runs the existing export script directly as a child process and
    /// waits for it to finish. It never launches Terminal or opens a window.
    static func refreshClaudeCredentialsForUsage() async -> Bool {
        await Task.detached(priority: .utility) {
            launchAuthBridge(
                script: shellScript(for: .claude, mode: .syncOnly),
                title: "Claude usage sync",
                hideTerminal: true,
                waitForExit: true
            )
        }.value
    ...
    private static func shellScript(for provider: UsageProviderID, mode: AuthMode) -> String {
        ...
        case .codex:
            return fileAuthLoginScript(
                ...
                pocketFile: "\(home)/Library/Application Support/Dynamite/UsageAuth/codex.auth.json",
        ...
        case .grok:
            return fileAuthLoginScript(
                ...
                pocketFile: "\(home)/Library/Application Support/Dynamite/UsageAuth/grok.auth.json",
    ...
    private static func claudeScript(exportPath: String, home: String, mode: AuthMode) -> String {
        ...
        // items without Dynamite ACL prompt. Writes files Pocket is allowed to read.
        let pocketDir = "\(home)/Library/Application Support/Dynamite/UsageAuth"
    ...
    @discardableResult
    private static func launchAuthBridge(
        script: String,
        title: String,
        hideTerminal: Bool,
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("Dynamite-provider-auth", isDirectory: true)
