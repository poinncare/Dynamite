...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func setupMenuBarItem() {
        ...
        statusItemVisibilityCancellable = Defaults.publisher(.menubarIcon)
            .sink { [weak self] change in
                self?.statusItem?.isVisible = change.newValue
            }
    }

    private func getUpdaterController() -> SPUStandardUpdaterController {
    ...
    @objc private func showSettingsFromMenu() {
        SettingsWindowController.shared.setUpdaterController(getUpdaterController())
        SettingsWindowController.shared.showWindow()
    }

    @objc private func checkForUpdatesFromMenu() {
        getUpdaterController().updater.checkForUpdates()
    }
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? DynamiteSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? DynamiteSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createDynamiteWindow(for screen: NSScreen, with viewModel: BoringViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = DynamiteSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        KeyboardShortcuts.onKeyDown(for: .clipboardHistoryPanel) { [weak self] in
            Task { [weak self] in
                ...
                // No auto-close timer for clipboard toggle (unlike toggleNotchOpen)
                self.closeNotchTask?.cancel()
                self.closeNotchTask = nil
        ...
        // Start clipboard history polling when enabled
        Task { @MainActor in
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createDynamiteWindow(
        ...
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    ...
                    let window = createDynamiteWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createDynamiteWindow(for: selectedScreen, with: vm)
            }
