# Pocket — спецификация интеграции (Dynamite + ядро Maccy, UI в стиле Pasta)

## Цель
Из клона Dynamite (`/Users/user/Dev/vibecode/Pocket/Dynamite`) сделать умный буфер обмена в шторке нотча:
история буфера обмена с функциональностью Maccy, отображаемая карточками в стиле Pasta, с навигацией стрелками и W/A/S/D.

Рабочая директория (единственный владелец записи): `/Users/user/Dev/vibecode/Pocket/Dynamite`.
...
### Dynamite
- Проект: `Dynamite.xcodeproj`, targets `Dynamite` + `DynamiteXPCHelper`, macOS 14.0, SPM.
- Entry: `@main DynamicNotchApp` + AppDelegate — `Dynamite/DynamiteApp.swift:15–52`; окно создаётся в `createDynamiteWindow` (App:234–265), класс `DynamiteSkyLightWindow` (NSPanel, styleMask `.borderless,.nonactivatingPanel,.utilityWindow,.hudWindow`) — `Dynamite/components/Notch/DynamiteSkyLightWindow.swift:34`.
...
1. Основа — существующий проект `Dynamite.xcodeproj`; НЕ создавать новый проект, НЕ переименовывать targets/bundle id.
2. Ядро Maccy портировать копированием файлов в новую папку `Dynamite/components/ClipboardHistory/` (подпапки Core/, Models/, Views/), адаптируя под Defaults 9.x и существующие конвенции проекта. UI Maccy не портировать.
...
4. Хранилище: SQLite в `Application Support/Dynamite/ClipboardHistory.sqlite` (не Maccy).
5. Новые SPM-зависимости: добавить Sauce (для paste) и fuse-swift (fuzzy поиск). Конфликтные дубликаты не добавлять — использовать уже имеющиеся Defaults/KeyboardShortcuts из Dynamite.
...
- Карточка (~140×120, скругление, тёмный материал в стиле Dynamite):
...
## Границы
- Не менять функциональность Home/Shelf/медиа-фич Dynamite.
...
1. `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project "Dynamite.xcodeproj" -scheme Dynamite -configuration Debug build` завершается успешно (exit 0), из корня `Dynamite`.
