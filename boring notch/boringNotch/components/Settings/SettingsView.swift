        ...
        NavigationSplitView {
            List(selection: $selectedTab) {
                ...
                NavigationLink(value: "Appearance") {
                    Label(L("Appearance"), systemImage: "eye")
                }
                NavigationLink(value: "Media") {
                    Label(L("Media"), systemImage: "play.laptopcomputer")
                }
                NavigationLink(value: "Calendar") {
                    Label(L("Calendar"), systemImage: "calendar")
                }
                NavigationLink(value: "HUD") {
                    Label(L("HUDs"), systemImage: "dial.medium.fill")
                }
                NavigationLink(value: "Battery") {
                    Label(L("Battery"), systemImage: "battery.100.bolt")
                }
//                NavigationLink(value: "Downloads") {
//                    Label(L("Downloads"), systemImage: "square.and.arrow.down")
//                }
                NavigationLink(value: "Shelf") {
                    Label(L("Shelf"), systemImage: "books.vertical")
                }
                ...
                //     Label(L("Extensions"), systemImage: "puzzlepiece.extension")
                // }
        ...
        .tint(.effectiveAccent)
        .id("\(accentColorUpdateTrigger)-\(language.revision)")
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                .tint(.effectiveAccent)
                LaunchAtLogin.Toggle(L("Launch at login"))
                Defaults.Toggle(key: .showOnAllDisplays) {
                    Text(L("Show on all displays"))
                }
                .onChange(of: showOnAllDisplays) {
                    NotificationCenter.default.post(
                        name: Notification.Name.showOnAllDisplaysChanged, object: nil)
                }
                Picker(L("Preferred display"), selection: $coordinator.preferredScreenUUID) {
                ...
                Defaults.Toggle(key: .automaticallySwitchDisplay) {
                    Text(L("Automatically switch displays"))
                }
            ...
            Section {
                Picker(
                    ...
                    label:
                        Text(L("Notch height on notch displays"))
                ) {
                    Text(L("Match real notch height"))
                        .tag(WindowHeightMode.matchRealNotchSize)
                    Text(L("Match menu bar height"))
                        .tag(WindowHeightMode.matchMenuBar)
                    Text(L("Custom height"))
                ...
                if notchHeightMode == .custom {
                    Slider(value: $notchHeight, in: 15...45, step: 1) {
                        Text(String(format: L("Custom notch size - %.0f"), notchHeight))
                    }
                ...
                Picker(L("Notch height on non-notch displays"), selection: $nonNotchHeightMode) {
                    Text(L("Match menubar height"))
                        .tag(WindowHeightMode.matchMenuBar)
                    Text(L("Match real notch height"))
                        .tag(WindowHeightMode.matchRealNotchSize)
                    Text(L("Custom height"))
                ...
                if nonNotchHeightMode == .custom {
                    Slider(value: $nonNotchHeight, in: 0...40, step: 1) {
                        Text(String(format: L("Custom notch size - %.0f"), nonNotchHeight))
                    }
    ...
    func gestureControls() -> some View {
        Section {
            Defaults.Toggle(key: .enableGestures) {
                Text(L("Enable gestures"))
            }
                .disabled(!openNotchOnHover)
            if enableGestures {
                Toggle(L("Change media with horizontal gestures"), isOn: .constant(false))
                    .disabled(true)
                Defaults.Toggle(key: .closeGestureEnabled) {
                    Text(L("Close gesture"))
                }
            ...
            HStack {
                Text(L("Gesture control"))
                customBadge(text: "Beta")
            }
        } footer: {
            Text(L("Two-finger swipe up on notch to close, two-finger swipe down on notch to open when **Open notch on hover** option is disabled"))
    ...
    func NotchBehaviour() -> some View {
        Section {
            Defaults.Toggle(key: .openNotchOnHover) {
                Text(L("Open notch on hover"))
            }
            Defaults.Toggle(key: .enableHaptics) {
                    Text(L("Enable haptic feedback"))
            }
            Toggle(L("Remember last tab"), isOn: $coordinator.openLastTabByDefault)
            if openNotchOnHover {
                Slider(value: $minimumHoverDuration, in: 0...1, step: 0.1) {
                    HStack {
                        Text(L("Hover delay"))
                        Spacer()
                        Text(String(format: L("%.1fs"), minimumHoverDuration))
                            .foregroundStyle(.secondary)
                    }
                }
        ...
        Form {
            Section {
                Defaults.Toggle(key: .showBatteryIndicator) {
                    Text(L("Show battery indicator"))
                }
                Defaults.Toggle(key: .showPowerStatusNotifications) {
                    Text(L("Show power status notifications"))
                }
            } header: {
                Text(L("General"))
            }
            Section {
                Defaults.Toggle(key: .showBatteryPercentage) {
                    Text(L("Show battery percentage"))
                }
                Defaults.Toggle(key: .showPowerStatusIcons) {
                    Text(L("Show power status icons"))
                }
            } header: {
                Text(L("Battery Information"))
            }
        }
        .onAppear {
            Task { @MainActor in
...
//                    Text(L("Show download progress"))
//                }
//                    .disabled(true)
//                Defaults.Toggle(key: .enableSafariDownloads) {
//                    Text(L("Enable Safari Downloads"))
//                }
//                    .disabled(!Defaults[.enableDownloadListener])
//                Picker(L("Download indicator style"), selection: $selectedDownloadIndicatorStyle) {
//                    Text(L("Progress bar"))
//                        .tag(DownloadIndicatorStyle.progress)
//                    Text(L("Percentage"))
//                        .tag(DownloadIndicatorStyle.percentage)
//                }
//                Picker(L("Download icon style"), selection: $selectedDownloadIconStyle) {
//                    Text(L("Only app icon"))
//                        .tag(DownloadIconStyle.onlyAppIcon)
//                    Text(L("Only download icon"))
...
struct HUD: View {
    ...
    var body: some View {
        Form {
            Section {
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(L("Replace system HUD"))
                            .font(.headline)
                        Text(L("Replaces the standard macOS volume, display brightness, and keyboard brightness HUDs with a custom design."))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                ...
                if !accessibilityAuthorized {
                    VStack(alignment: .leading, spacing: 8) {
                        Text(L("Accessibility access is required to replace the system HUD."))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)

                        HStack(spacing: 12) {
                            Button(L("Request Accessibility")) {
            ...
            Section {
                Picker(L("Option key behaviour"), selection: $optionKeyAction) {
                    ForEach(OptionKeyAction.allCases) { opt in
                        Text(opt.rawValue).tag(opt)
                    }
                }
                
                Picker(L("Progress bar style"), selection: $enableGradient) {
                    Text(L("Hierarchical"))
                        .tag(false)
                    Text(L("Gradient"))
                        .tag(true)
                ...
                Defaults.Toggle(key: .systemEventIndicatorShadow) {
                    Text(L("Enable glowing effect"))
                }
                Defaults.Toggle(key: .systemEventIndicatorUseAccent) {
                    Text(L("Tint progress bar with accent color"))
                }
            } header: {
                Text(L("General"))
            }
            ...
            Section {
                Defaults.Toggle(key: .showOpenNotchHUD) {
                    Text(L("Show HUD in open notch"))
                }
                Defaults.Toggle(key: .showOpenNotchHUDPercentage) {
                    Text(L("Show percentage"))
                }
            ...
            Section {
                Picker(L("HUD style"), selection: $inlineHUD) {
                    Text(L("Default"))
                        .tag(false)
                    Text(L("Inline"))
                        .tag(true)
...
struct Media: View {
    ...
    var body: some View {
        Form {
            Section {
                Picker(L("Music Source"), selection: $mediaController) {
                ...
                if MusicManager.shared.isNowPlayingDeprecated {
                    HStack {
                        Text(L("YouTube Music requires this third-party app to be installed: "))
                            .foregroundStyle(.secondary)
                            .font(.caption)
                        Link(
                    ...
                    Text(L("'Now Playing' was the only option on previous versions and works with all media apps."))
                    .foregroundStyle(.secondary)
                    .font(.caption)
                }
            }
            ...
            Section {
                Toggle(L("Show music live activity"),
                    isOn: $coordinator.musicLiveActivityEnabled.animation()
                )
                Toggle(L("Show sneak peek on playback changes"), isOn: $enableSneakPeek)
                Picker(L("Sneak Peek Style"), selection: $sneakPeekStyles) {
                ...
                HStack {
                    Stepper(value: $waitInterval, in: 0...10, step: 1) {
                        HStack {
                            Text(L("Media inactivity timeout"))
                            Spacer()
                            Text(String(format: L("%.0f seconds"), Defaults[.waitInterval]))
                                .foregroundStyle(.secondary)
                        }
                    }
                }
                Picker(
                    ...
                    label:
                        HStack {
                            Text(L("Full screen behavior"))
                            customBadge(text: "Beta")
                        }
                ) {
                    Text(L("Hide for all apps")).tag(HideNotchOption.always)
                    Text(L("Hide for media app only")).tag(
                        HideNotchOption.nowPlayingOnly)
                    Text(L("Never hide")).tag(HideNotchOption.never)
                }
            } header: {
                Text(L("Media playback live activity"))
            }
            ...
            Section {
                ...
                Defaults.Toggle(key: .enableLyrics) {
                    HStack {
                        Text(L("Show lyrics below artist name"))
                        customBadge(text: "Beta")
                    }
                }
            } header: {
                Text(L("Media controls"))
            }  footer: {
                Text(L("Customize which controls appear in the music player. Volume expands when active."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
        .accentColor(.effectiveAccent)
        .navigationTitle(L("Media"))
    }
    ...
    var body: some View {
        Form {
            Defaults.Toggle(key: .showCalendar) {
                Text(L("Show calendar"))
            }
            Defaults.Toggle(key: .hideCompletedReminders) {
                Text(L("Hide completed reminders"))
            }
            Defaults.Toggle(key: .hideAllDayEvents) {
                Text(L("Hide all-day events"))
            }
            Defaults.Toggle(key: .autoScrollToNextEvent) {
                Text(L("Auto-scroll to next event"))
            }
            Defaults.Toggle(key: .showFullEventTitles) {
                Text(L("Always show full event titles"))
            }
            Section(header: Text(L("Calendars"))) {
                if calendarManager.calendarAuthorizationStatus != .fullAccess {
                    Text(L("Calendar access is denied. Please enable it in System Settings."))
                        .foregroundColor(.red)
                        .multilineTextAlignment(.center)
                        .padding()
                    Button(L("Open Calendar Settings")) {
            ...
            Section(header: Text(L("Reminders"))) {
                if calendarManager.reminderAuthorizationStatus != .fullAccess {
                    Text(L("Reminder access is denied. Please enable it in System Settings."))
                        .foregroundColor(.red)
                        .multilineTextAlignment(.center)
                        .padding()
                    Button(L("Open Reminder Settings")) {
                        if let settingsURL = URL(
                            string:
...
struct About: View {
    ...
    var body: some View {
        VStack {
            Form {
                Section {
                    ...
                    HStack {
                        ...
                        Spacer()
                        if showBuildNumber {
                            Text(String(format: L("(%@)"), Bundle.main.buildVersionNumber ?? ""))
                                .foregroundStyle(.secondary)
                        }
            ...
            VStack(spacing: 0) {
                Divider()
                Text(L("Made with 🫶🏻 by not so boring not.people"))
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
        ...
        Form {
            Section {
                ...
                Defaults.Toggle(key: .openShelfByDefault) {
                    Text(L("Open shelf by default if items are present"))
                }
                Defaults.Toggle(key: .expandedDragDetection) {
                    Text(L("Expanded drag detection area"))
                }
                ...
                Defaults.Toggle(key: .copyOnDrag) {
                    Text(L("Copy items on drag"))
                }
                Defaults.Toggle(key: .autoRemoveShelfItems) {
                    Text(L("Remove from shelf after dragging"))
                }
            ...
            Section {
                Picker(L("Quick Share Service"), selection: $quickShareProvider) {
                ...
                if let selectedProvider = selectedProvider {
                    HStack {
                        ...
                        .foregroundColor(.accentColor)
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(format: L("Currently selected: %@"), selectedProvider.id))
                                .font(.caption)
                ...
                Text(L("Choose which service to use when sharing files from the shelf. Click the shelf button to select files, or drag files onto it to share immediately."))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        }
        .accentColor(.effectiveAccent)
        .navigationTitle(L("Shelf"))
    }
}
...
struct Appearance: View {
    ...
    var body: some View {
        Form {
            Section {
                Toggle(L("Always show tabs"), isOn: $coordinator.alwaysShowTabs)
                Defaults.Toggle(key: .settingsIconInNotch) {
                    Text(L("Show settings icon in notch"))
                }
            ...
            Section {
                ...
                Defaults
                    .Toggle(L("Player tinting"), key: .playerColorTinting)
                Defaults.Toggle(key: .lightingEffect) {
                    Text(L("Enable blur effect behind album art"))
                }
                Picker(L("Slider color"), selection: $sliderColor) {
            ...
            Section {
                Toggle(L("Use music visualizer spectrogram"),
                    isOn: $useMusicVisualizer.animation()
                )
                .disabled(true)
                if !useMusicVisualizer {
                    if customVisualizers.count > 0 {
                        Picker(L("Selected animation"),
                        ...
                        HStack {
                            Text(L("Selected animation"))
                            Spacer()
                            Text(L("No custom animation available"))
                                .foregroundStyle(.secondary)
                        }
                    }
                }
                ...
                HStack {
                    Text(L("Custom music live activity animation"))
            ...
            Section {
                ...
                .sheet(isPresented: $isPresented) {
                    VStack(alignment: .leading) {
                        ...
                        HStack {
                            Text(L("Speed"))
                            Spacer(minLength: 80)
                            Text(String(format: L("%.1fs"), speed))
                                .multilineTextAlignment(.trailing)
                                .foregroundStyle(.secondary)
                ...
                HStack(spacing: 0) {
                    Text(L("Custom vizualizers (Lottie)"))
                    if !Defaults[.customVisualizers].isEmpty {
                        Text(String(format: L(" – %d"), Defaults[.customVisualizers].count))
                            .foregroundStyle(.secondary)
                    }
                }
            }
            ...
            Section {
                ...
                Defaults.Toggle(key: .showNotHumanFace) {
                    Text(L("Show cool face animation while inactive"))
                }
            } header: {
                HStack {
                    Text(L("Additional features"))
                }
            }
        }
        .accentColor(.effectiveAccent)
    ...
    var body: some View {
        Form {
            Section {
                    ...
                    if !useCustomAccentColor {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(L("Using System Accent"))
                                        .font(.body)
                                    Text(L("Your macOS system accent color"))
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                                Spacer()
                ...
                Text(L("Choose between your system accent color or customize it with your own selection."))
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
            ...
            Section {
                Defaults.Toggle(key: .enableShadow) {
                    Text(L("Enable window shadow"))
                }
                Defaults.Toggle(key: .cornerRadiusScaling) {
                    Text(L("Corner radius scaling"))
                }
            } header: {
                Text(L("Window Appearance"))
            }
            ...
            Section {
                Defaults.Toggle(key: .extendHoverArea) {
                    Text(L("Extend hover area"))
                }
                Defaults.Toggle(key: .hideTitleBar) {
                    Text(L("Hide title bar"))
                }
                Defaults.Toggle(key: .showOnLockScreen) {
                    Text(L("Show notch on lock screen"))
                }
                Defaults.Toggle(key: .hideFromScreenRecording) {
                    Text(L("Hide from screen recording"))
                }
            } header: {
                Text(L("Window Behavior"))
            }
        }
        .accentColor(.effectiveAccent)
        ...
        Form {
            Section {
                Defaults.Toggle(key: .clipboardEnabled) {
                    Text(L("Enable clipboard history"))
                }
                ...
                Defaults.Toggle(key: .clipboardPasteAutomatically) {
                    Text(L("Paste on single click"))
                }

                HStack {
                    Text(L("History size"))
                    Spacer()
                    Stepper(value: $clipboardHistorySize, in: 1...9) {
                        Text("\(clipboardHistorySize)")
                            .monospacedDigit()
                            .frame(minWidth: 24, alignment: .trailing)
                    }
                    .onChange(of: clipboardHistorySize) { _, newValue in
                        let clamped = min(max(newValue, 1), 9)
                        if clamped != newValue {
                            clipboardHistorySize = clamped
                        }
                    }
                }

                HStack {
                    Text(L("Check interval (s)"))
                    Spacer()
                ...
                Text(L("When “Paste on single click” is off, click only copies; Enter and double-click still paste. History is stored in Application Support/boringNotch/ClipboardHistory.sqlite"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            Section {
                Defaults.Toggle(key: .clipboardIgnoreEvents) {
                    Text(L("Ignore clipboard events"))
                }
                ...
                Text(L("Whitelist uses the ignored-apps list as an allow list. “Ignore only next event” turns off ignore after one skipped copy."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            Section {
                Button(L("Clear unpinned history"), role: .destructive) {
                    ClipboardHistoryManager.shared.clearUnpinned()
                }
            }
        }
        .formStyle(.grouped)
        .accentColor(.effectiveAccent)
        .navigationTitle(L("Clipboard"))
    }
}
...
struct Shortcuts: View {
    var body: some View {
        Form {
            Section {
                KeyboardShortcuts.Recorder(L("Toggle Sneak Peek:"), name: .toggleSneakPeek)
            } header: {
                Text(L("Media"))
            } footer: {
                Text(L("Sneak Peek shows the media title and artist under the notch for a few seconds."))
                .multilineTextAlignment(.trailing)
                .foregroundStyle(.secondary)
                .font(.caption)
            }
            Section {
                KeyboardShortcuts.Recorder(L("Toggle Notch Open:"), name: .toggleNotchOpen)
                KeyboardShortcuts.Recorder(L("Open Clipboard History:"), name: .clipboardHistoryPanel)
            }
        }
        .accentColor(.effectiveAccent)
        .navigationTitle(L("Shortcuts"))
    }
}
